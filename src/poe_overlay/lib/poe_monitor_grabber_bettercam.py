...
class MonitorGrabber:
    ...
    def grab_geometry(self, window):
        """grab_geometry"""
        x1, y1, x2, y2 = window
        haystack = self.sct.grab(window)
        if haystack is not None:
            self.image_general = haystack
        else:
            if not hasattr(self, "image_general"):
                self.image_general = np.zeros((y2-y1, x2-x1, 3), dtype=np.uint8)
        time.sleep(0.05)
    ...
    def shrink_game_window(self):
        """shrink_game_window"""
        image_sh = np.array(self.image_or[:, ::6, 2], dtype="int32") # type: ignore
    ...
    def shrink_game_windowg(self):
        """shrink_game_windowg"""
        image_sh = np.array(self.image_general[:, ::6, 2], dtype="int32") # type: ignore
        image_sh[image_sh == 0] = 1
        image_sh_res = image_sh[:, :-1] * image_sh[:, 1:]
        image_sh_res = image_sh_res[:, :-1] * image_sh[:, 2:]
        image_sh_res = image_sh_res[:, :-1] * image_sh[:, 3:]
