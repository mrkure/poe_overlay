...
class MouseWorkers:
    ...
    @staticmethod
    def wheel_forward(worker, params={"hotkey": "wf","active": True, "timeout": 1, "flasks": [[2,3,4]]}):
        """wheel_forward"""
        if not worker:
            params["_flasks_pointer"] = 0
            return params
...
class AutomationWorkers:
    ...
    @staticmethod
    def automana(widget, mana_value, game_active, pprint=False, params={"active": True, "keys": [[5]], "low_lim": 1, "high_lim": 70, "timeout": 5000}):
        """wheel_forward"""
        if pprint:
